"""[DEPRECATED 兼容 shim] 真实实现已搬到 backend.modules.skill_loader。

本 shim 保留旧 build_system_prompt 的多桶切片逻辑(身份卡/数据快照/回答规则),
因为旧行为已被生产 agent 调优,直接换成新模块的简化版会改变 system prompt 结构。
新代码应直接用:from backend.modules.skill_loader import load_skill_prompt
"""
...
# 9 个 Few-shot 案例目录(每个文件是一个真实场景对话)
_CONVERSATIONS_DIR = None  # 延迟初始化


def _get_conversations_dir() -> Path | None:
    """Few-shot 对话目录 = skill_dir/references/conversations/"""
    global _CONVERSATIONS_DIR
    if _CONVERSATIONS_DIR is None:
        _CONVERSATIONS_DIR = settings.skill_dir / "references" / "conversations"
    return _CONVERSATIONS_DIR if _CONVERSATIONS_DIR.is_dir() else None


def _load_few_shot_examples() -> str:
    """加载 9 个 Few-shot 案例对话,作为 system prompt 末尾的模仿样本。

    设计:
    - 抽"对话示例"段(## 开头的对话),跳过头部元提示
    - 每个文件只摘核心对话,不让 prompt 爆炸
    - 文件名前缀数字 01-09 控制顺序
    - 任何 { 字符都已验证(9 个文件无 {} 配对),无需转义
    """
    ...
    blocks = ["\n\n# Few-shot 真实对话示例(模仿对象)\n",
             "以下 9 个真实场景对话展示张雪峰的回答风格——先反问关键信息、引用具体数据、给冲稳保 3 档、必须给行动项。\n"
             "**你必须严格模仿这种对话流,不要变成普通 AI 助手口吻。**\n"]
    ...
    for f in files:
        ...
        # 抽"真实对话示例"段(从 ## 真实对话示例 到文件末尾,或 ## 关键提示词 之前)
        m = re.search(r"## 真实对话示例\n+(.*?)(?=^## 关键提示词|\Z)", text, re.MULTILINE | re.DOTALL)
        body = m.group(1).strip() if m else text

        # 截断超长对话(> 2500 字)— 留前 2500 字避免 prompt 爆炸
        if len(body) > 2500:
            body = body[:2500] + "\n\n(对话截断,完整版见参考文件)"
...
def build_system_prompt() -> str:
    """Compose system prompt — 保留旧的多桶切片实现以维持生产行为稳定。"""
    text = _load_skill_text()
    sections = _split_by_h2(text) if text else []

    skill_core = _grab(sections, ["身份卡", "核心心智", "决策启发式", "表达 DNA", "表达DNA", "心智模型"])
    data_snapshot = _grab(sections, ["2026 福建", "福建", "选科铁律", "投档线", "数据基准", "院校"])
    answer_rules = _grab(sections, ["回答工作流", "角色扮演", "红线", "回答规则"])
...
"""
    ...
    ) + multi_turn_rule + _load_few_shot_examples()
...
__all__ = ["build_system_prompt", "warmup"]
